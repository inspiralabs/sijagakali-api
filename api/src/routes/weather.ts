...
export async function registerWeatherRoutes(app: FastifyInstance, deps: RouteDeps) {
  ...
  async function loadActiveDevices(slug: string): Promise<DeviceWeatherRow[]> {
    const { data, error } = await supabase
      .from('device_configs')
      .select('device_id, location_name, display_name, bmkg_adm4, bmkg_nowcast_keywords')
  ...
  app.get<{
    Querystring: { deployment_slug?: string };
  }>('/api/weather/nowcast', async (req, reply) => {
    const slug = req.query.deployment_slug ?? ENV.DEFAULT_DEPLOYMENT_SLUG;
    ...
    const keywords = collectNowcastKeywords(devices);

    try {
      const allAlerts = await getNowcastAlerts();
      const relevant = filterNowcastByKeywords(allAlerts, keywords);
      const othersCount = allAlerts.length - relevant.length;

      return reply.send({
        deployment_slug: slug,
        alerts: relevant,
        total_active: allAlerts.length,
        other_regions_count: Math.max(0, othersCount),
      ...
      const msg = e instanceof Error ? e.message : String(e);
      req.log.warn({ msg: 'BMKG nowcast error', error: msg });
      return reply.code(502).send({ error: msg });
    }
  });
  ...
  app.patch<{
    ...
    const { bmkg_adm4, bmkg_nowcast_keywords } = req.body;
    ...
    const parsedKw = parseKeywords(bmkg_nowcast_keywords);
    if (bmkg_nowcast_keywords !== undefined) {
      if (!Array.isArray(bmkg_nowcast_keywords)) {
        return reply.code(400).send({ error: 'bmkg_nowcast_keywords harus array string' });
      }
      updates.bmkg_nowcast_keywords = parsedKw ?? [];
    }
